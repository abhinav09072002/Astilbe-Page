...
import {
  PlatformPage, SolutionsPage, ResourcesPage,
  EnterprisesPage, AdNetworkingPage, PricingPage,
} from './components/Pages.jsx'

// FIX: Use the central api.js helpers instead of duplicating fetch calls here.
// This ensures the proxy/VITE_API_URL logic stays in one place.
import { apiSubscribe, apiGetCount } from './utils/api.js'
...
function HomePage() {
  ...
  // FIX: use the central helper so the URL is consistent
  useEffect(() => {
    apiGetCount()
  ...
  const handleSubmit = useCallback(async (email, source) => {
    ...
    setSubmitting(true)
    try {
      const data = await apiSubscribe({ email: email.trim().toLowerCase(), source })
      if (data.success || data.alreadySubscribed) {
        const norm = email.trim().toLowerCase()
        setSubscribers(prev =>
          prev.find(s => s.email === norm)
            ? prev
            : [...prev, { email: norm, ts: Date.now() }]
        )
